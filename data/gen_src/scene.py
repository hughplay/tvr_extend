...
class Scene:
    ...
    def obj_to_camera_view(self, obj, scene, camera, width, height, vertex):
        vertex_location = Vector(vertex.co) * obj.matrix_world + obj.location
        x, y, _ = bpy_extras.object_utils.world_to_camera_view(
            scene, camera, vertex_location)
        view_x =int(round(x * width))
        view_y =int(round(y * height))
        return view_x, view_y

    def get_bound_box(self, obj):
        bound_box = {}
        for name, camera in self.cameras.items():
            scene = bpy.context.scene
            scale = scene.render.resolution_percentage / 100.
            ...
            bbox = camera_view_bounds_2d(scene, camera, obj)

            # points = [
            #     self.obj_to_camera_view(
            #         obj, scene, camera, width, height, vertex)
            ...
            # #     view_x =int(round(x * width))
            # #     view_y =int(round(y * height))
            # #     x_list.append(view_x)
            # #     y_list.append(view_y)
            # bbox = (min(x_list), min(y_list), max(x_list), max(y_list))
            # bound_box[name] = bbox
            bound_box[name] = bbox.to_tuple()
        return bound_box
