...
class Renderer:
    ...
    def prepare_log_and_dir(self):
        ...
        self.output_dir = Path(
            self.config.output_data_dir).expanduser()
    ...
    def prepare_basic_settings(self):
        ...
        self.global_seed = utils.hash_seed(
            self.config.label, self.config.seed)
    ...
    def load_scene(self, show=False):
        if show:
    ...
    def images_path(self):
        ...
        return {
            cam : os.path.join(
                str(self.image_dir), 'img_{:06d}-{}.{}.png'.format(
                    self.current_idx, self.str_stage, cam.split('_')[1][0]))
            for cam in cameras}
    ...
    def segs_path(self):
        return {
            cam : os.path.join(
            str(self.seg_dir), 'seg_{:06d}-{}.{}.png'.format(
                self.current_idx, self.str_stage, cam.split('_')[1][0]))
    ...
    def blend_path(self):
        return os.path.join(
            str(self.blend_dir), '{:06d}-{}.blend'.format(
                self.current_idx, self.str_stage))

    @property
    def old_json_path(self):
        return os.path.join(str(self.info_dir), 'TRANCE_standard_{:06d}.json'.format(
            self.current_idx))

    @property
    def json_path(self):
        return os.path.join(str(self.info_dir), '{:06d}.json'.format(
            self.current_idx))
    ...
    def run(self, replace_start=-1):
        for self.current_idx in tqdm.trange(self.start, self.end, ncols=80):
            ...
            if os.path.isfile(self.old_json_path):
                info = self.sample_manager.record_json(self.old_json_path)
                if self.current_idx >= replace_start:
                    # self.render_from_json(info)
                    self.update_objects_to_json(info)
                continue
    ...
    def render_from_json(self, info):
        ...
        lamps = info['lamps']
        self.num_step = len(info['transformations'])
    ...
    def update_objects_to_json(self, info):
        cameras = info['cameras']
        lamps = info['lamps']
        for s in info['states']:
            objects = s['objects']
            self.scene.set_scene(objects, cameras, lamps)
            s['objects'] = deepcopy(self.scene.objects_description)
...
if __name__ == '__main__':
    config = utils.Config('configs/trance.yaml')
