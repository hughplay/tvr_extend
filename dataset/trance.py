...
class TRANCE(Dataset):
    ...
    def prepare_image_and_boxes(
        self, initial_img_name, final_img_name, init_boxes=None, fin_boxes=None
    ):
        ...
        if init_boxes is not None:
            ...
            init_boxes = self._resize_boxes(
                init_boxes, self.ORIGIN_SIZE, real_size
            )
            fin_boxes = self._resize_boxes(
        ...
        if self.img_aug:
            (initial_state, final_state), (
                ...
                imgs=(initial_state, final_state), boxes=(init_boxes, fin_boxes)
            )
    ...
    def transform(self, imgs, boxes, translation=0.05):
        ...
        if boxes[0] is not None:
            boxes = [
                np.clip(
                    s_boxes + np.array([*translate, *translate]),
                    a_min=0,
                    a_max=self.ORIGIN_SIZE * 2,
                    dtype=s_boxes.dtype
                )
    ...
    def _resize_boxes(self, boxes, origin_size, real_size):
        scale_x = real_size[0] / origin_size[0]
        scale_y = real_size[1] / origin_size[1]
        boxes = boxes * np.array([scale_x, scale_y, scale_x, scale_y]).astype(
            boxes.dtype
        )
        return boxes

    def img2arr(self, *imgs):
        return [np.array(img)[..., :3].transpose([2, 0, 1]) for img in imgs]
...
class Basic(TRANCE):
    ...
    def __getitem__(self, idx):
        ...
        init_img, fin_img, init_boxes, fin_boxes = self.prepare_image_and_boxes(
            info["init_img_name"],
            info["fin_img_name"],
            info["init_boxes"],
            info["fin_boxes"],
        ...
        if self.use_box:
            sample = {
                **sample,
                "init_boxes": init_boxes,
                "fin_boxes": fin_boxes,
                "n_init": init_boxes.shape[0],
                "n_fin": fin_boxes.shape[0],
            }
...
class TRANCEDataModule(LightningDataModule):
    ...
    def train_dataloader(self):
        train_dataset = self.data_cls(
            ...
            move_out_aug=self.move_out_aug,
