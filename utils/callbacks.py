...
class EarlyStopping(Callback):
    ...
    def _run_early_stopping_check(self, trainer: "pl.Trainer") -> None:
        ...
        should_stop, reason = self._evalute_stopping_criteria(current, trainer.current_epoch)
    ...
    def _evalute_stopping_criteria(self, current: torch.Tensor, current_epoch: int) -> Tuple[bool, str]:
        should_stop = False
        reason = None
        ...
        if self.min_epochs > 0 and current_epoch <= self.min_epochs:
            should_stop = False
            reason = f"Haven't reach min training epochs: {self.min_epochs}"
